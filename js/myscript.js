...
function countdownTimer() {
	...
	for (var cnt = 0; cnt < startTime.length; cnt++) {
		document.getElementById(toLocalID[cnt]).innerHTML = "อีเว้นท์จะเริ่มเวลา: <br>" + startTime[cnt].local().format("YYYY-MM-DD HH:mm:ss") + " " + moment.tz(moment.tz.guess()).zoneAbbr();
	}
	continueTimer();
	function continueTimer() {
		...
		for (var cnt = 0; cnt < startTime.length; cnt++) {
			...
			if (distance >= 0) {
				...
				document.getElementById(timerID[cnt]).innerHTML = "จะเริ่มใน: " + ("0" + days).slice(-2) + "d " + ("0" + hours).slice(-2) + "h " + ("0" + minutes).slice(-2) + "m " + ("0" + seconds).slice(-2) + "s ";		
			}
			if (distance == -1) {
				globalConfetti();
			}
			...
			if (distance < startTime[cnt].unix() - stopTime[cnt].unix()) {
				...
				document.getElementById(timerID[cnt]).innerHTML = "อีเว้นท์นี้จบแล้ว";
			}
		}
